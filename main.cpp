...
HttpCallback(HINTERNET, DWORD_PTR ctx, DWORD status, LPVOID info, DWORD infoLen) {
	...
	case WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE: {
		...
		if (WinHttpQueryHeaders(
			state->request, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER,
	...
	case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE: {
		...
		if (bytesAvailable == 0) {
			...
			// 非同期読み取りを要求
			if (!WinHttpReadData(
				state->request, state->inflightBuffer.data(), bytesAvailable, nullptr)) {
				// 読み取り開始に失敗
				state->error = true;
		...
		if (info) {
			if (infoLen > 0) {
				...
				state->responseBody.append(
					state->inflightBuffer.data(), state->inflightBuffer.data() + infoLen);
...
static bool StartHttpRequest(
	...
	state.session = WinHttpOpen(
		L"realtime-rest-check/1.0", WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME,
	...
	state.request = WinHttpOpenRequest(
		state.connect, method, path, nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
		WINHTTP_FLAG_SECURE);
	if (!state.request) {
		state.error = true;
		state.errorCode = GetLastError();
	...
	BOOL ok = WinHttpSendRequest(
		state.request, headers, static_cast<DWORD>(-1), (LPVOID)body, bodyLength, bodyLength, 0);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// 行1: 操作ボタンとフェーズ表示
		{
			bool canConnect =
				!asyncState.active &&
				(asyncState.phase == HttpPhase::Idle || asyncState.phase == HttpPhase::Completed ||
					asyncState.phase == HttpPhase::Error || asyncState.phase == HttpPhase::Canceled);

			if (canConnect) {
				if (ImGui::Button("Connect")) {
					StartHttpRequest(
						asyncState, L"GET", kHostName, kPath, kHttpHeaders, nullptr, 0);
				}
				ImGui::SameLine();
		...
		if (asyncState.phase == HttpPhase::Error) {
			...
			ImGui::TextDisabled(
				"request=%p  connect=%p  session=%p", asyncState.request, asyncState.connect,
				asyncState.session);
			...
			ImVec4 col = (asyncState.statusCode >= 200 && asyncState.statusCode < 300)
				? ImVec4(0.3f, 1.0f, 0.5f, 1.0f)
				: ImVec4(1.0f, 0.7f, 0.2f, 1.0f);

			ImGui::TextColored(
				col, "HTTP %lu (Completed, %zu bytes)", asyncState.statusCode, asyncState.totalRead);
			...
			ImGui::InputTextMultiline(
				"##json", pretty.data(), pretty.size(), ImVec2(0, 260), ImGuiInputTextFlags_ReadOnly);
			ImGui::PopTextWrapPos();

		}
		else if (
			asyncState.phase == HttpPhase::Sending || asyncState.phase == HttpPhase::Waiting) {

			ImGui::TextColored(ImVec4(0.9f, 0.9f, 0.2f, 1.0f), "Waiting response...");
			ImGui::BulletText(
				asyncState.phase == HttpPhase::Sending ? "Sending request..."
				: "Awaiting headers...");
			ImGui::TextDisabled(
				"request=%p connect=%p session=%p", asyncState.request, asyncState.connect,
				asyncState.session);
