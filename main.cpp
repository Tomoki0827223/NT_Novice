...
const char kWindowTitle[] = "LE3C_ムラタ_トモキ";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
  ...
  "messages": [
	...
	HINTERNET session = WinHttpOpen(
		L"realtime-rest-check/1.0", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY, nullptr, nullptr, 0);
	if (!session) {
	...
	HINTERNET hRequest = WinHttpOpenRequest(
		connect, L"POST", kConnectionPath, nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
		WINHTTP_FLAG_SECURE);
	if (!hRequest) {
	...
	BOOL ok = WinHttpSendRequest(
		hRequest, kHttpHeaders, static_cast<DWORD>(-1), static_cast<LPVOID>(const_cast<char*>(kBody)),
