...
static bool StartHttpRequest(
	HttpAsyncState& state, const wchar_t* host, const wchar_t* path, const wchar_t* headers,
	const char* body) {
	if (state.active) {
		return false;
	}
	...
	state.request = WinHttpOpenRequest(
		state.connect, L"POST", path, nullptr, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES,
		WINHTTP_FLAG_SECURE);
	if (!state.request) {
		state.error = true;
		state.errorCode = GetLastError();
	...
	const DWORD kCallbackFlags = WINHTTP_CALLBACK_FLAG_SENDREQUEST_COMPLETE |
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// 行1: 操作ボタンとフェーズ表示
		{
			bool canConnect =
				!asyncState.active &&
				(asyncState.phase == HttpPhase::Idle || asyncState.phase == HttpPhase::HeadersDone ||
